/*  
    SQL Script: Create and Populate the 'startups' Table  
    Description: A dataset of company startups used for SQL query practice.  
...
-- 1. CREATE TABLE
------------------------------------------------------------
-- Run this first (SQL Server 2016+)
DROP TABLE IF EXISTS dbo.startups;
GO
...
-- Max raised among Seed-stage startups
SELECT
    MAX(raised) AS max_raised_seed
FROM startups
WHERE stage = 'Seed';
...
-- Average valuation (all startups)
SELECT
...
-- Average valuation by category (rounded)
SELECT
    category,
...
-- Average valuation by category (rounded + ordered)
SELECT
    category,
    ROUND(AVG(valuation), 2) AS avg_valuation
FROM startups
...
-- Locations with average employees > 500
-- (Use HAVING with the aggregate expression for broad compatibility)
SELECT
    location,
    AVG(employees) AS avg_employees
FROM startups
